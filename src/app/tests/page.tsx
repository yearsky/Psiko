...
import { BookOpen, Clock, Star, Filter, Sparkles } from 'lucide-react'
import { dummyPackages } from '@/data/dummy'
import { StudyWithAIModal } from '@/components/ai/StudyWithAIModal'
    ...
    <div className="space-y-8">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <button
          ...
          className="text-left block bg-white rounded-2xl border border-blue-200 p-5 hover:shadow-md hover:border-blue-300 transition-all group"
        >
          ...
          <div className="mb-1">
            <span className="text-xs font-medium text-blue-700">AI Learning</span>
          </div>
          ...
          <p className="text-sm text-gray-500 mb-4 line-clamp-2">
            AI akan mengenali kebutuhanmu lewat 3 pertanyaan, lalu memberi 1 soal latihan yang sesuai fokusmu.
          </p>
          ...
          <div className="flex items-center gap-2 flex-wrap pt-3 border-t border-gray-100">
            ...
            <span className="text-xs text-gray-400 flex items-center gap-1">
              <Sparkles className="w-3 h-3" /> 3 pertanyaan awal
            </span>
          </div>
        </button>
      ...
      {isStudyModalOpen && <StudyWithAIModal onClose={() => setIsStudyModalOpen(false)} />}
    </div>
  )
}
